                        ...
                        <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                            ...
                            <td class=" px-5 py-3">
                                <div class="w-48 flex items-center gap-3">
                                    <img src="https://i.pravatar.cc/150?u={contact?.name}" alt={contact?.name} width="40px" height="40px"/>
                                    <div class="data">
                                        <p class="font-normal text-sm text-gray-900">{contact?.name}</p>
                                    </div>
                                </div>
                            </td>
							<td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 "> {contact.company}</td>
                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {contact.job}</td>
                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {contact.email}
                            </td>
							<td class="flex p-5 items-center gap-0.5">
								<form action="?/delete" method="post" use:enhance>
                    			<input type="hidden" hidden name="id" value={contact?.id}>
                                  <button class="p-2 cursor-pointer rounded-full bg-white group transition-all duration-500 hover:bg-red-600 flex item-center">
